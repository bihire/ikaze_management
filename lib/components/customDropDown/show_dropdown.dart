...
///  * [showCustomDropDown], which actually displays the dialog and returns its result.
///  * <https://material.io/design/components/dialogs.html>
class Dialog extends StatelessWidget {
  /// Creates a dialog.
  ///
  /// Typically used in conjunction with [showCustomDropDown].
  const Dialog({
    Key key,
    this.backgroundColor,
    this.elevation,
    this.insetAnimationDuration = const Duration(milliseconds: 1000),
    this.insetAnimationCurve = Curves.easeInOut,
...
///
/// Typically passed as the child widget to [showCustomDropDown], which displays the
/// dialog.
///
...
class SimpleDialogOption extends StatelessWidget {
  ...
  /// with a value for [showCustomDropDown] to complete its future with.
...
/// options are represented as an enum. The [showCustomDropDown] method here returns
...
///   switch (await showCustomDropDown<Department>(
///     context: context,
///     builder: (BuildContext context) {
///       return SimpleDialog(
...
Widget _buildMaterialDialogTransitions(
  ...
  return Transform.translate(
    child: FadeTransition(
        ...
        child: child),
    offset: Offset(0.0, animation.value * 4),
  );
}
...
/// `showCustomDropDown` is originally called from. Use a [StatefulBuilder] or a
...
Future<T> showCustomDropDown<T>({
  ...
  return Navigator.of(context, rootNavigator: useRootNavigator)
    ...
    context: context,
    builder: builder,
    // popUpLocationdata: PopUpLocationdata(
    height: height,
    width: width,
    xPosition: xPosition,
    yPosition: yPosition,
...
///
/// It is used internally by [showCustomDropDown] or can be directly pushed
/// onto the [Navigator] stack to enable state restoration. See
/// [showCustomDropDown] for a state restoration app example.
///
/// This function takes a `builder` which typically builds a [Dialog] widget.
/// Content below the dialog is dimmed with a [ModalBarrier]. The widget
/// returned by the `builder` does not share a context with the location that
...
/// See also:
///
///  * [showCustomDropDown], which is a way to display a DialogRoute.
///  * [showGeneralDialog], which allows for customization of the dialog popup.
///  * [showCupertinoDialog], which displays an iOS-style dialog.
