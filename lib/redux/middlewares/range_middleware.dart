...
Future<List<MoneyTransactionModel>> _loadRangeTransactions(
  ...
  var response = await http.get(
      'http://172.17.2.204:5000/api/transactions?page=$page&numberOfRows=$perPage');
...
Future<RangeCountModel> _loadRangeCount(
  ...
  var response = await http.get(
      'http://172.17.2.204:5000/api/transactions/range/count?fromdate=${Utils.formatToDate(fromDate)}&todate=${Utils.formatToDate(toDate)}');
  if (response.statusCode == 200) {
    final jsonData = (json.decode(response.body))['data'] as List;
